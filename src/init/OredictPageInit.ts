...
export class OredictPageInit extends Init {
  canRun() {
    return !!(
      this.parent.href.includes("/oredict/") && this.configs.getSettings("advancedOredictPage")
    );
  }
  ...
  run() {
    ...
    const sortInput = $(
      `<input id="mcmodder-mod-search" placeholder="输入模组名称或编号以筛选..." class="form-control">`,
    ).appendTo(sortFrame);
    ...
    $(".oredict-item-list li").each((_, _li) => {
      ...
      const modID = Utils.abstractLastFromURL(
        li.find("div.sub.class a").first().prop("href"),
        "class",
      );
    ...
    sortContainer.children().each((_, _fieldset) => {
      ...
      list.css(
        ...
        `repeat(min(${count}, var(--mcmodder-oredict-column)), 1fr)`,
      );
    });
