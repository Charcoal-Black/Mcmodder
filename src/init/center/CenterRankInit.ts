...
export class CenterRankInit extends CenterBaseInit {
  private refreshExpBar(newExp: number, currentExp = 0) {
    ...
    if (level <= Values.maxLevel) {
      $(".lv-title span:nth-child(2)").html(
        `升级进度: ${newExp.toLocaleString()} / ${Values.expRequisition[level].toLocaleString()} Exp`,
      );
    ...
    $(".lv-title span:nth-child(1)").html(
      `${s > currentExp ? "预测等级" : "当前等级"}: <i class="common-user-lv large lv-${level}">Lv.${level}</i>`,
    );
  ...
  run() {
    ...
    const progressExp = parseInt(
      $(".lv-title > span:nth-child(2)").text().replace("升级进度: ", "").replace(",", ""),
    );
    lvTitle.innerHTML +=
      '<span>升至<i class="common-user-lv large lv-' +
      Math.min(currentLevel + 1, Values.maxLevel) +
      '">Lv.<input id="mcmodder-lv-input" maxlength="2"></i> 还需经验: <span id="mcmodder-expreq" style="margin-right: 0px">-</span> Exp</span>';
    ...
    if (progressExp < 1e5)
      for (let i = 1; i < currentLevel; i++) totalExp += Values.expRequisition[i];
    ...
    const editNumEvents = {
      change: (e: JQueryEventObject) => {
        ...
        for (
          let i = Math.floor((this.center.pageProfile.editNum + 1) / 1e3) * 1e3;
          i <= Math.min(1.9e4, Math.floor(value / 1e3 - 1) * 1e3);
          i += 1e3
        ) {
          t += (i + 1e3) / 2;
        }
        for (
          let i = Math.floor((this.center.pageProfile.editByte + 1) / 5e4) * 5e4;
          i <= Math.min(9.5e5, Math.floor($("#mcmodder-editbyte").val() / 5e4 - 1) * 5e4);
          i += 5e4
        ) {
          t += (i + 5e4) / 100;
        }
    ...
    const editByteEvents = {
      change: (e: JQueryEventObject) => {
        ...
        for (
          let i = Math.floor((this.center.pageProfile.editByte + 1) / 5e4) * 5e4;
          i <= Math.min(9.5e5, Math.floor(value / 5e4 - 1) * 5e4);
          i += 5e4
        ) {
          t += (i + 5e4) / 100;
        }
        for (
          let i = Math.floor((this.center.pageProfile.editNum + 1) / 1e3) * 1e3;
          i <= Math.min(1.9e4, Math.floor(Number($("#mcmodder-editnum").val()) / 1e3 - 1) * 1e3);
          i += 1e3
        ) {
          t += (i + 1e3) / 2;
        }
    ...
    $(lvTitle).append(
      `<span>总经验: <span id="mcmodder-totalexp" style="margin-right: 0px">${totalExp.toLocaleString()} Exp</span></span><span>次数计算器: <input id="mcmodder-editnum"></span><span>字数计算器: <input id="mcmodder-editbyte"></span>`,
    );
    $("#mcmodder-editnum")
      .val(this.center.pageProfile?.editNum || "")
      .change((e) => editNumEvents.change(e))
      .keydown((e) => editNumEvents.keydown(e));
    $("#mcmodder-editbyte")
      .val(this.center.pageProfile?.editByte || "")
      .change((e) => editByteEvents.change(e))
      .keydown((e) => editByteEvents.keydown(e));

    // 若未加载主页，则禁用输入框
    if (!this.center.pageProfile)
      $("#mcmodder-editnum, #mcmodder-editbyte").attr({
        disabled: "disabled",
