...
export class ItemEditorInit extends Init {
  canRun() {
    return this.parent.href.includes("/item/edit/") || this.parent.href.includes("/item/add/");
  }
    ...
    if (ctx)
      img.onload = () => {
        canvas.width = canvas.height =
          Number(t) *
          ($(".common-item-mold-list li a[data-category-selected='true']").attr(
            "data-multi-value",
          ) === "6"
            ? 1.125
            : 1);
  ...
  run() {
    ...
    $('<input id="icon-32x-editor" placeholder="输入 Base64 格式小图标...">').insertAfter(
      "#icon-32x",
    );
    $('<input id="icon-128x-editor" placeholder="输入 Base64 格式大图标...">').insertAfter(
      "#icon-128x",
    );
    $('<button class="btn">同步至大图标</button>')
      .insertAfter("#icon-32x-editor")
      .click(() => this.imgResize(32, 128));
    $('<button class="btn">同步至小图标</button>')
      .insertAfter("#icon-128x-editor")
      .click(() => this.imgResize(128, 32));
    ...
    // JSON 快速手导
    const jsonUploader = $(
      '<input id="mcmodder-json-upload" class="mcmodder-monospace form-control" placeholder="粘贴 JSON 物品导出行于此处以快速填充基本信息..">',
    )
      ...
      .change(() => {
        ...
        try {
          ...
          if (e instanceof SyntaxError)
            Utils.commonMsg("请检查提交的 JSON 语法是否正确~", false, "解析错误");
          return;
        }
        try {
          ...
          $("#icon-128x")
            ...
            .change();
          if (data.OredictList)
            data.OredictList.slice(1, data.OredictList.length - 1)
              .split(", ")
              .forEach((e) => {
