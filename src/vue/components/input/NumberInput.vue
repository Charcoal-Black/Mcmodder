...
const { valueRef, onChange, getInstance, getValue, setCurrentValue, setDisplayValue } =
  useInputBase({
    inputRef,
    value: props.value,
    validate,
...
function validate(newValue: number): InputValidInfo<number> {
  ...
  if (!isNaN(min) && newValue < min)
    return {
      isok: false,
      msg: `您输入的数值 (${newValue.toLocaleString()}) 低于允许的最小值 (${min.toLocaleString()})，请重新设置~`,
    };
  if (!isNaN(max) && newValue > max)
    return {
      isok: false,
      msg: `您输入的数值 (${newValue.toLocaleString()}) 高于允许的最大值 (${max.toLocaleString()})，请重新设置~`,
    };
