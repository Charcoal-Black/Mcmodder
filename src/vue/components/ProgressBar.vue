...
const DISPLAYRULE_PERCENT: ProgressBarDisplayRule = (val, min, max) =>
  `${Utils.getPrecisionFormatter(0, 0).format(((val - min) / (max - min)) * 100)}%`;
const DISPLAYRULE_FRACTION: ProgressBarDisplayRule = (val, _min, max) =>
  `${val.toLocaleString()} / ${max.toLocaleString()}`;

const displayRule = computed(
  () => props.displayRule ?? ((props.max ?? 1) === 1 ? DISPLAYRULE_PERCENT : DISPLAYRULE_FRACTION),
);
