...
onMounted(() => {
  ...
  const preSubmitList = props.parent.configRepository.getProfile("preSubmitList") || [] as PreSubmission[];
  ...
  $(table.root).on("click", ".mcmodder-presubmit-delete", e => {
    ...
    if (errState) html += `上一次试图提交此预编辑项时遇到错误：“${ Values.errorMessage[errState] }”，在此处修改后可尝试重新提交。<br>`
