    ...
    <div class="edit-autolink-frame" ref="classSearchFrame">
      ...
      <div class="edit-autolink-style">
        <div class="checkbox">
          ...
          <label :for="`jsonframe_${ id }-importclass-infer`">访问潜在资料 - 在一轮资料列表获取完毕后，考虑到同一类资料通常是在同一个批次中批量添加的，脚本会试图访问那些可能仍然属于目标模组区域，但是未出现在现有资料列表中的物品资料 ID。这种方法能够应对综合子资料数量大于 100 的情况，以及访问到部分隐藏分类中的资料。</label>
        </div>
        <div class="checkbox">
          <input :id="`jsonframe_${ id }-importclass-geticon`" name="geticon" type="checkbox">
          <label :for="`jsonframe_${ id }-importclass-geticon`">保存物品图标 - 读取的同时获取物品的小图标和大图标，并以 Base64 格式保存进 JSON 文件里。启用该项配置会显著增大输出文件体积；若不启用，则在显示物品图标时会实时从百科获取图标。</label>
        </div>
        <div class="checkbox">
          <input :id="`jsonframe_${ id }-importclass-getall`" name="getall" type="checkbox">
          <label :for="`jsonframe_${ id }-importclass-getall`">保存完整数据 - 读取一个资料的全部数据（包括图标、注册名、物品标签等所有可以在编辑页访问的数据）。启用该项配置会忽略“保存物品图标”的配置。确切来说，脚本会通过逐一访问所有物品的编辑页来获取这些数据。<strong>启用此项将会向服务器发送大量请求，使用前请务必妥善配置脚本“最短发包间隔”！！</strong></label>
        </div>
      </div>
...
onMounted(() => {
  ...
  .addItem({
    ...
    displayRule: e => {
      ...
      return !!(
        index >= 0 && table.getEditorData(index, "id") &&
  ...
  jsonFrame.value!.addTool("export", "导出当前文件至本地", () => !!frame.activeFileName, () => exportJson(frame.activeFileName));
  jsonFrame.value!.addTool("importClass", "从模组导入JSON", () => true, () => openClassSearchFrame())
  jsonFrame.value!.addTool("importOnline", "从收纳贴导入JSON", () => true, () => searchOnlineFiles())
  jsonFrame.value!.addTool("submitedit", "提交所有改动至百科", () => !!(frame.activeFileName && table.unsaved), () => frame.submitEdit(), true);
})
...
async function getItemListByClassID(config: ItemJsonFrameConfig) {
  ...
  for (let i in branchList) {
    ...
    await getItemListFromPage(`${ props.parent.hostname }/item/list/${ branchList[i] }.html`, itemList, branchNameList[i], config);
...
async function performClassSearch(classID: number, typeID: number) {
  ...
  $(classSearchFrame.value!).find("input[name]").each((_, _input) => {
