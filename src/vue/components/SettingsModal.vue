                    ...
                    <div v-for="s in g.sections" :key="s.id" class="group-card">
                      <div class="group-title-row">
                        ...
                        <template v-if="s.id === 'update'">
                          ...
                          <Timer
                            v-if="getValue('autoCheckUpdate')"
                            class="mcmodder-settings-timer"
                            :parent="parent"
                            :data-getter="updateTimerGetter"
                          />
                        </template>
                      </div>

                      <div v-if="s.id === 'data'" class="data-manager">
                        ...
                        <div class="center-content mcmodder-storage">
                          <ul>
                            <li v-for="resourceProps in configResourceInteractorProps" :key="resourceProps.id">
                              <ConfigResourceInteractor
                                :ref="el => setSplashManagerRef(resourceProps.id, el)"
                                v-bind="resourceProps"
                              >
                                <template v-if="resourceProps.id === SPLASH_STORAGE_ID" #default>
                                  <button
                                    ref="splashCompareButton"
                                    class="btn"
                                    :id="Mcmodder.ID_SPLASH_COMPARE"
                                    @click="comparePublicSplash"
                                  >与公共标语库对比</button>
                                </template>
                              </ConfigResourceInteractor>
                            </li>
                            <li v-for="fileProps in configResourceFileListInteractorProps" :key="fileProps.id">
                              <ConfigResourceFileListInteractor v-bind="fileProps" />
                            </li>
                          </ul>
                        </div>
                        ...
                        <div class="center-setting-block" style="margin-top: 2em;">
                          ...
                          <div class="setting-item" style="display: flex; gap: 8px; align-items: center;">
                            <input v-model="splashInput" type="text" class="form-control" placeholder="输入自定义闪烁标语内容..." style="max-width: 400px; display: inline-block;">
                            <button ref="splashSubmitButton" class="btn btn-primary" @click="submitSplash">提交投稿</button>
                          </div>
                        </div>
                      ...
                      <div v-for="entry in s.keys" :key="entry" class="setting-item">
                        ...
                        <div class="setting-control">
                          ...
                          <SupabaseAuthBinder
                            v-if="entry === 'useSupabase' && !!getValue('useSupabase')"
                            :parent="parent"
                          />
                        </div>
                      </div>
...
import type {
  ...
  McmodderClassRelationData,
  McmodderConfigData,
  McmodderRankDisplayData,
  McmodderRankStorageData,
  McmodderSplashData
} from "../../types";
...
const updateTimerGetter = McmodderTimer.DATAGETTER_SCHEDULE("autoCheckUpdate", null, parent.scheduleRequestUtils);

function checkUpdate() {
  parent.scheduleRequestUtils.run("autoCheckUpdate");
}

const SPLASH_STORAGE_ID = "mcmodderSplashList_v2";

const configResourceInteractorProps = [
  {
    parent: parent,
    id: SPLASH_STORAGE_ID,
    name: "已记录的闪烁标语",
    headConfigs: {
    ...
    configParser: (config: string) => config?.split("\n") || [], // 最后一项是空，不考虑
    dataParser: (_: string, data: string) => {
  ...
  } satisfies ConfigResourceInteractorProps<McmodderSplashData>,
  {
    ...
    dataParser: (key: string, item: any) => ({
      id: key,
      children: item
    })
  ...
  } satisfies ConfigResourceInteractorProps<McmodderClassRelationData>,
  {
    ...
    dataParser: (key: string, item: string) => {
...
] as const satisfies readonly ConfigResourceFileListInteractorProps<any>[];

const splashManager = ref<{ table: { getAllData(): McmodderSplashData[] } } | null>(null);
const splashCompareButton = useTemplateRef<HTMLButtonElement>("splashCompareButton");
const splashSubmitButton = useTemplateRef<HTMLButtonElement>("splashSubmitButton");

function setSplashManagerRef(id: string, el: any) {
  if (id === SPLASH_STORAGE_ID) splashManager.value = el;
}

async function comparePublicSplash() {
  const button = splashCompareButton.value;
  if (button) McmodderUtils.setButtonLoadingState(button);
  try {
    await accessPublicSplashList();
  } finally {
    if (button) McmodderUtils.cancelButtonLoadingState(button);
  }
}

async function accessPublicSplashList() {
  const data = (splashManager.value?.table.getAllData() ?? [])
    .map(item => item.content).filter(item => item) as string[];
  if (!data.length) {
...
async function submitSplash() {
  ...
  const button = splashSubmitButton.value;
  if (button) McmodderUtils.setButtonLoadingState(button);
...
<style>
/* 数据管理：资源管理器内容由子组件（Collapsible / GenericTable）渲染，scoped
   样式无法命中其内部元素，需以非 scoped 形式注入（vite 构建时统一标记并由
   syncVueStyles 复制进 Shadow Root）。 */
