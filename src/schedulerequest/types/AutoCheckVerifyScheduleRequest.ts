  ...
  private async work(adminModList: string[], inVerifyPage: boolean) {
    ...
    if (inVerifyPage) {
      ...
      menuOptions.each((index, node) => {
        ...
        if (elem.dataset.divider) { // 审核助理分割线下不属于自己的管理区域，不考虑
          return false;
        }
    ...
    for (const id of adminModList) {
      ...
      const count = Number($(JSON.parse(resp.responseText).html).find(".selectJump").next().text().slice(4, -2).replaceAll(",", ""));
      if (count && inVerifyPage) {
        ...
        firstChild.append(`<span class="mcmodder-admin-verify-notify text-danger">${ count }个待审！</span>`).removeClass("disabled");
      }
      total += count;
    }
    return total;
