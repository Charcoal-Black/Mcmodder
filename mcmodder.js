...
// @exclude      https://www.mcmod.cn/modpack/*
// @exclude      https://play.mcmod.cn/
...
function customStyle() {
	let e = (e, t) => {
		...
		let n = ((i & 65280) >> 8) - o;
		let l = (i & 255) - o;
		return "#" + (0 | Math.max(Math.min(a, 255), 0) << 16 | Math.max(Math.min(n, 255), 0) << 8 | Math.max(Math.min(l, 255), 0))
			.toString(16)
			.padStart(6, "0")
	};
	let t = getConfig("themeColor1") || "#86c155",
		...
		d = a + "80",
		c = n + "80";
	addStyle(":root {--mcmodder-tc1:" + t + "; --mcmodder-tc2:" + i + "; --mcmodder-tc3:" + o + "; --mcmodder-td1:" + a + "; --mcmodder-td2:" + n + "; --mcmodder-tca1:" + l + "; --mcmodder-tca2:" + r + "; --mcmodder-tda1:" + d + "; --mcmodder-tda2:" + c + "; --mcmodder-bgcolor: rgba(255,255,255," + Math.max(Math.min(parseInt(getConfig("backgroundAlpha") || 204), 255), 128) / 255 + "); --mcmodder-txcolor: #333; --mcmodder-platform-forge: #5b6197; --mcmodder-platform-fabric: #8a7b71; --mcmodder-platform-neoforge: #dc895c; --mcmodder-platform-quilt: #8b61b4; --mcmodder-platform-liteloader: #4c90de;}");
...
function tabInit() {
	...
	const n = $(".common-nav li:not(.line):nth-child(5) a")
		.text();
	let l = getConfig(e, "modDependences")
		.split(","),
		r = getConfig(e, "modExpansions")
		.split(",");
	...
	for (let e of n) {
		...
		switch (r) {
			...
			case "slot-power-number":
				o[d].id = $("p.title", e)
					.html()
					.split("<span")[0], o[d].number = l.value, o[d].valid = true, o[d].unit = c
		}
	}
	$(".item-table-gui-slot")
		.each((e, t) => {
...
let textCompare = function(e, t) {
	...
	let o = ($(".difference-info")[0] || $("#verify-window-frame")[0] || $(".tab-content")[0])
	...
	.insertBefore(o, $(".difference-area")[0] || $(".verify-info-table")[0] || $("#edit-page-1")[0]);
	let a = function(e) {
		let t = "";
		e.contents()
			.filter(function(e, t) {
		...
		for (let h of e) {
			if (h.type === "childList" && h ? .addedNodes[0] ? .className === "swal2-container swal2-center swal2-fade swal2-shown" && $("h2#swal2-title")
				...
				p.filter((e, t) => t.className != "limit" && t.className != "empty")
					.each(function() {
						autoLinkIndex = 1, i = $("a", this)[0], a = i.title.split(" - ")[2] || "", n = a.includes("[") ? a.slice(1)
							.split("] ")[0] : a.split(" (")[0], l = $(i)
							.attr("data-id");
						...
						let e = getConfig(c, "modDependences")
							.split(","),
							t = getConfig(c, "modExpansions")
							.split(",");
						if (a === modName || n === "MC" || e.includes(a) || t.includes(a) || m.includes(n.toLowerCase()) || m.includes("原版") || $("a", this)
							...
							if (m.includes(r.toLowerCase()) || m.includes(d.toLowerCase()) || $("a", this)
								.attr("data-type") === "oredict") {
				...
				$(".edit-autolink-list ul")
					...
					.each((e, t) => {
						if (e > 0 && e < 10) $("a", t)
							.append(`<span class="mcmodder-common-light item-ename"> [Alt+${e}]</span>`)
					});
...
function itemEditorInit() {
	...
	$(document.createElement("input"))
		...
		.insertAfter(e);
	$(document.createElement("input"))
		.attr("id", "icon-32x-editor")
		.insertAfter("#icon-32x");
	$(document.createElement("input"))
		.attr("id", "icon-128x-editor")
	...
	$("#icon-32x, #icon-128x")
		.bind("change", function() {
			let e = $(this)
				.val();
			if (e.slice(0, 11) != "data:image/") $(this)
...
function classInit() {
	...
	$(".class-info-left > ul > *, .common-link-frame .title")
		.contents()
		.filter((e, t) => t.nodeType === 3 && (["相关链接:", "模组标签: ", "支持的MC版本: "].includes(t.data) || t.data.includes("作者/开发团队")))
		.each(function() {
			$(this)
				.parent()
				.find("> i")
	...
	$(".common-center > .right")
		.remove();
	let n = parseInt($(".infolist.modpack")
		.text()
		.slice(2));
	let l = parseInt($(".infolist.server-count")
		.text()
		.slice(2));
	let r = parseFloat($(".infolist.server-pre")
		.text()
		.split("安装率为 ")[1]);
	let d = parseFloat($(".infolist.worldgen")
		.text()
		.split("有 ")[1]);
	$(".infolist.modpack, .infolist.server-count, .infolist.server-pre, .infolist.worldgen")
		.remove();
	if (n) $(`<li class="col-lg-6"><span class="title">${n}</span><span class="text"><a target="_blank" href="/modpack.html?mod=${e}">整合包已收录</a></span></li>`)
		.insertAfter($(".col-lg-6")
			.last());
	if (l) $(`<li class="col-lg-6"><span class="title">${l}</span><span class="text"><a target="_blank" href="https://play.mcmod.cn/list/?classid=${e}">服务器已安装</a></span></li>`)
		.insertAfter($(".col-lg-6")
			.last());
	if (r) $(`<li class="col-lg-6"><span class="title">${r}%</span><span class="text">模组服安装率</span></li>`)
		.insertAfter($(".col-lg-6")
			.last());
	if (d) $(`<li class="col-lg-6"><span class="title">${d}</span><span class="text"><a target="_blank" href="https://www.mcmod.cn/worldgen.html?mod=${e}">资源分布数据</a></span></li>`)
		.insertAfter($(".col-lg-6")
			.last());
				...
				$($(".select-row")[0].appendChild(document.createElement("button")))
					...
					.bind("click", function() {
						...
						let r = async function(e) {
							...
							if (o > 0 && l === 0) $("button.btn:nth-child(2)")
								.click();
							l += o;
							if (o > 0) {
...
function rankInit() {
	...
	function e(e) {
		...
		for (let r in d) {
			...
			let a = $("a", d[r])[0].href;
			let n = parseFloat($("span.score", d[r])
				.text());
			if (isNaN(n)) n = 0;
			f += n;
			let l = $("a.name", d[r])
			...
			i.style.float = "left";
			i.style.backgroundImage = 'url("' + $("img", d[r])
				.attr("src")
				.replace() + '")';
			...
			i.style.width = "75%";
			if (r > 0 && $(d[r])
				.attr("data-content") != $(d[r - 1])
				.attr("data-content")) g = r;
			switch (parseInt(g)) {
				case 0:
...
function centerInit() {
	...
	const t = new MutationObserver(function(t, a) {
		for (let e of t) {
			if (e.addedNodes.length > 1) {
				...
				a.disconnect();
				$($("div.center-block[data-menu-frame=9] div.center-setting-block:nth-child(7)")[0].appendChild(document.createElement("pre")))
	...
	const o = new MutationObserver(function(t, n) {
		for (let e of t) {
			if (e.addedNodes[0].className = "center-main lv" && $(".lv-title")
				...
				$(e)
					.append(`<span>总经验: <span id="mcmodder-totalexp" style="margin-right: 0px">${a.toLocaleString()} Exp</span></span><span>次数计算器: <input id="mcmodder-editnum" style="height: 2em;"></span><span>字数计算器: <input id="mcmodder-editbyte" style="height: 2em;"></span>`);
				$("#mcmodder-editnum")
					.val(c);
				$("#mcmodder-editbyte")
					.val(s);
	...
	const n = new MutationObserver(function(t, r) {
		for (let e of t) {
			if (e.addedNodes[0] ? .className === "center-total") {
				...
				i.remove();
				o = t.appendChild(document.createElement("li"));
				o.innerHTML = '<span class="title">科龄</span><span class="text">' + parseInt((Date.now() - Date.parse($(".center-total li:nth-child(7) span.text")
						.text())) / (24 * 60 * 60 * 1e3))
	...
	if (l != "none") $("div.bbs-link")
		.append(`<p align="right"><a href="${l.replace('url("',"").replace('")',"")}" target="_blank">查看个人中心背景图片</a></p>`);
	$("div.bbs-link")
...
function commentInit() {
	...
	if (getConfig("autoCheckUpdate")) {
		...
		setTimeout(async function() {
			let e = async function(e) {
				let t = await fetch("https://bbs.mcmod.cn/forum.php?mod=viewthread&tid=20483", {
					method: "GET"
				});
				let i = $("<html>")
					.html(await t.text()),
					o;
				let a = i.find("#postmessage_85878 font[size=5]")
					.first()
					.text()
					.split("Mcmodder v")[1].split(" --")[0];
				if (n(mcmodderVersion, a) < 0) {
					let e = i.find("#postmessage_85878 .spoilerbody")
						.first()
						.html();
					o = "https://bbs.mcmod.cn/" + i.find(".attnm a")
						.first()
						.attr("href");
					Swal.fire({
							type: "info",
	...
	$(".col-lg-12.common-rowlist-2 li, .class-text-top .col-lg-4")
		.each(function(i) {
			let e = $(this)
				.text();
